...
class Pointnet2Backbone(nn.Module):
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, end_points=None):
        ...
        features = self.fp1(end_points['sa3_xyz'], end_points['sa4_xyz'], end_points['sa3_features'],
                            end_points['sa4_features'])  # （512， 256）
        ...
        features = self.fp2(end_points['sa2_xyz'], end_points['sa3_xyz'], end_points['sa2_features'],
                            features)  # （1024， 256）
...
class CrissCrossAttention(nn.Module):
    ...
    def __init__(self, in_dim, ratio=8):
        ...
        self.query_conv = nn.Sequential(
            nn.Conv2d(in_channels=in_dim, out_channels=in_dim // ratio, kernel_size=1, bias=False),
            self.bn1,
            nn.ReLU())
