...
def collect_point_label(anno_path, out_filename, file_format='txt'):
  ...
  #房间里每一个instance的点放在一起，用文件名的第一个单词来标明，例如wall_(eg:office_1......)
  #glob.glob()方法返回所有匹配的文件路径表
  #os.path.basename(path)返回对应的文件名
  ...
  for f in glob.glob(os.path.join(anno_path, '*.txt')):
    ...
    points_list.append(np.concatenate([points, labels], 1)) # Nx7  (num of instances)x(num of points)x7
  
  # np.amin()用于计算数组中元素沿指定轴的最小值，0--每列最小值，1--每行最小值
  # np.save()函数将数组保存到以.npy为扩展名的文件中
  data_label = np.concatenate(points_list, 0) # ((num of instances)x(num of points))x7
...
def point_label_to_obj(input_filename, out_filename, label_color=True, easy_view=False, no_wall=False):
  ...
  out_filename: OBJ filename,
    ...
    easy_view: do not visualize air conditioner
  """
...
#   """
#   # assert()断言，用于判断一个表达式，为true时程序正常运行，为false时触发异常
...
def room2samples(data, label, sample_num_point):
  ...
  Returns:
    sample_datas: K x sample_num_point x 6
           numpy array of XYZRGB, RGB is in [0,1]
