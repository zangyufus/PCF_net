# This script is to produce training data for semantic segmentation using S3DIS dataset
import os
import numpy as np
import sys
import json
...
indoor3d_data_dir = os.path.join(data_dir, 'tud_facade')
NUM_POINT = 4096
H5_BATCH_SIZE = 1000
...
# Set paths
filelist = os.path.join(BASE_DIR, 'meta/all_data_label_facade.txt')
data_label_files = [os.path.join(indoor3d_data_dir, line.rstrip()) for line in open(filelist)]
# output_dir = os.path.join(data_dir, 'indoor3d_sem_seg_hdf5_data_test') # generate test data
# output_dir = os.path.join(data_dir, 'indoor3d_sem_seg_hdf5_data_train') # generate training data
output_dir = os.path.join(data_dir, 'facade_sem_seg_hdf5_data_train') # generate training data
...
for i in range(h5_index):
    all_file.write(os.path.join('facade_sem_seg_hdf5_data_train', 'ply_data_all_') + str(i) +'.h5\n')
all_file.close()
