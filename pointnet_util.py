...
def index_points(points, idx):
    ...
    """
    raw_size = idx.size() # 16 1024 16
    idx = idx.reshape(raw_size[0], -1) # 16 16384
    res = torch.gather(points, 1, idx[..., None].expand(-1, -1, points.size(-1))) # 16 16384 C
...
def farthest_point_sample(xyz, npoint):
    ...
    """
    device = xyz.device
    B, N, C = xyz.shape # 16 1024 3
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device) # 16 256
    distance = torch.ones(B, N).to(device) * 1e10 # 16 1024
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device) # 16
    batch_indices = torch.arange(B, dtype=torch.long).to(device) # 16
    for i in range(npoint):
        centroids[:, i] = farthest # 16 256
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 3) # 16 1 3
        dist = torch.sum((xyz - centroid) ** 2, -1) # 16 1024
        distance = torch.min(distance, dist) # 16 1024
        farthest = torch.max(distance, -1)[1] #16
    return centroids
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, knn=False):
    ...
    B, N, C = xyz.shape # 16 1024 3
    S = npoint # 256
    fps_idx = farthest_point_sample(xyz, npoint) # [B, npoint]16 256
    torch.cuda.empty_cache()
    new_xyz = index_points(xyz, fps_idx) # 16 256 3
    torch.cuda.empty_cache()
    if knn:
